...
module address(
  ...
  /* PAR MK3 wrapper overrides — driven from parmk3_top in main.v.
   * When sel_mk3_bios/sel_mk3_sram is high the ROM_ADDR is forced to the
   * BIOS / MK3-SRAM reservation regions; otherwise the standard game-mapper
   * logic below produces the address. */
  input parmk3_sel_bios,
  input parmk3_sel_mk3sram,
  input parmk3_sel_game,
  input [14:0] parmk3_sram_offset,
  input [16:0] parmk3_bios_offset
);
...
/* PAR MK3 wrapper rebases ROM_ADDR into the MCU-reserved BIOS / MK3 SRAM
 * regions when the wrapper claims the access. Otherwise the base mapper
 * (HiROM/LoROM/...) computes the game-cart address as usual. The constants
 * here MUST match SRAM_PARMK3_BIOS_ADDR and SRAM_PARMK3_MK3RAM_ADDR in
 * src/memory.h. */
assign ROM_ADDR = parmk3_sel_bios
                  ? (24'hB00000 | {7'h0, parmk3_bios_offset})
                  : parmk3_sel_mk3sram
                  ? (24'hB20000 | {9'h0, parmk3_sram_offset})
                  : SRAM_SNES_ADDR;

assign ROM_HIT = IS_ROM | IS_WRITABLE | bs_page_enable
                 | parmk3_sel_bios | parmk3_sel_mk3sram;
